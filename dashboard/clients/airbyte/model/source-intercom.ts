/* tslint:disable */
/* eslint-disable */
/**
 * airbyte-api
 * Programatically control Airbyte Cloud, OSS & Enterprise.
 *
 * The version of the OpenAPI document: 1.0.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */



/**
 * 
 * @export
 * @interface SourceIntercom
 */
export interface SourceIntercom {
    /**
     * UTC date and time in the format 2017-01-25T00:00:00Z. Any data before this date will not be replicated.
     * @type {string}
     * @memberof SourceIntercom
     */
    'start_date': string;
    /**
     * Access token for making authenticated requests. See the <a href=\"https://developers.intercom.com/building-apps/docs/authentication-types#how-to-get-your-access-token\">Intercom docs</a> for more information.
     * @type {string}
     * @memberof SourceIntercom
     */
    'access_token': string;
    /**
     * Client Id for your Intercom application.
     * @type {string}
     * @memberof SourceIntercom
     */
    'client_id'?: string;
    /**
     * Client Secret for your Intercom application.
     * @type {string}
     * @memberof SourceIntercom
     */
    'client_secret'?: string;
    /**
     * Set lower value in case of failing long running sync of Activity Logs stream.
     * @type {number}
     * @memberof SourceIntercom
     */
    'activity_logs_time_step'?: number;
    /**
     * 
     * @type {string}
     * @memberof SourceIntercom
     */
    'sourceType': SourceIntercomSourceTypeEnum;
}

export const SourceIntercomSourceTypeEnum = {
    INTERCOM: 'intercom'
} as const;

export type SourceIntercomSourceTypeEnum = typeof SourceIntercomSourceTypeEnum[keyof typeof SourceIntercomSourceTypeEnum];


