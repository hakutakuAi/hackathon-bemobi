/* tslint:disable */
/* eslint-disable */
/**
 * airbyte-api
 * Programatically control Airbyte Cloud, OSS & Enterprise.
 *
 * The version of the OpenAPI document: 1.0.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */


// May contain unused imports in some cases
// @ts-ignore
import type { SourceDatadogQueriesInner } from './source-datadog-queries-inner';

/**
 * 
 * @export
 * @interface SourceDatadog
 */
export interface SourceDatadog {
    /**
     * Datadog API key
     * @type {string}
     * @memberof SourceDatadog
     */
    'api_key': string;
    /**
     * Datadog application key
     * @type {string}
     * @memberof SourceDatadog
     */
    'application_key': string;
    /**
     * The search query. This just applies to Incremental syncs. If empty, it\'ll collect all logs.
     * @type {string}
     * @memberof SourceDatadog
     */
    'query'?: string;
    /**
     * UTC date and time in the format 2017-01-25T00:00:00Z. Any data before this date will not be replicated. This just applies to Incremental syncs.
     * @type {string}
     * @memberof SourceDatadog
     */
    'start_date'?: string;
    /**
     * The site where Datadog data resides in.
     * @type {string}
     * @memberof SourceDatadog
     */
    'site'?: SourceDatadogSiteEnum;
    /**
     * UTC date and time in the format 2017-01-25T00:00:00Z. Data after this date will  not be replicated. An empty value will represent the current datetime for each  execution. This just applies to Incremental syncs.
     * @type {string}
     * @memberof SourceDatadog
     */
    'end_date'?: string;
    /**
     * Maximum number of records to collect per request.
     * @type {number}
     * @memberof SourceDatadog
     */
    'max_records_per_request'?: number;
    /**
     * List of queries to be run and used as inputs.
     * @type {Array<SourceDatadogQueriesInner>}
     * @memberof SourceDatadog
     */
    'queries'?: Array<SourceDatadogQueriesInner>;
    /**
     * 
     * @type {string}
     * @memberof SourceDatadog
     */
    'sourceType': SourceDatadogSourceTypeEnum;
}

export const SourceDatadogSiteEnum = {
    DATADOGHQ_COM: 'datadoghq.com',
    US3_DATADOGHQ_COM: 'us3.datadoghq.com',
    US5_DATADOGHQ_COM: 'us5.datadoghq.com',
    DATADOGHQ_EU: 'datadoghq.eu',
    DDOG_GOV_COM: 'ddog-gov.com'
} as const;

export type SourceDatadogSiteEnum = typeof SourceDatadogSiteEnum[keyof typeof SourceDatadogSiteEnum];
export const SourceDatadogSourceTypeEnum = {
    DATADOG: 'datadog'
} as const;

export type SourceDatadogSourceTypeEnum = typeof SourceDatadogSourceTypeEnum[keyof typeof SourceDatadogSourceTypeEnum];


